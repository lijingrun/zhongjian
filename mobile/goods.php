...
// Login only vip members.
//if( $_SESSION['user_id'] == 0 || $_SESSION['user_rank'] == 1 ) { $smarty->display('login.html'); exit; }

$goods_id = !empty($_GET['id']) ? intval($_GET['id']) : '';
$err = get_liked_member($goods_id);
...
$smarty->assign('error', $err['err_msg']);
...
function get_liked_member($goods_id){
   $sql="select * from " .$GLOBALS['ecs']->table('goods_user'). "where goods_id=$goods_id";
   $goods=$GLOBALS['db']->getAll($sql);
   if(!empty($goods)){
      $sql="select * from " .$GLOBALS['ecs']->table('goods_user'). " where goods_id=$goods_id and user_id='". $_SESSION['user_id']."'";
      $goods_res =$GLOBALS['db']->getOne($sql);
      if(empty($goods_res)){
