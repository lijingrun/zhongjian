...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $db->query($sql);
		 ...
		 /* 只允许访问的会员 QQ:1986219154*/
        $goods_member_list = array();
        $sql = "DELETE FROM " . $ecs->table('goods_user') .
                " WHERE goods_id = 0 AND admin_id = '$_SESSION[admin_id]'";
        $db->query($sql);
    ...
    else
    {
		...
		$goods_member_list = get_goods_member($goods['goods_id']);   // 只允许访问的会员 QQ：1986219154
	...
	$smarty->assign('goods_member_list', $goods_member_list);//QQ:1986219154
...
elseif ($_REQUEST['act'] == 'get_member_list')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    {
        $sql     .=  " Where user_name LIKE '%" . mysql_like_quote(trim($filters['user_name'])) . "%' ";
    }
...
elseif ($_REQUEST['act'] == 'add_goods_member')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $user_name   = $json->decode($_GET['add_ids']);
    $arguments  = $json->decode($_GET['JSON']);
    $goods_id   = $arguments[0];

    foreach ($user_name AS $val)
    {
        $sql = "INSERT INTO " . $ecs->table('goods_user') . " (goods_id, user_id, admin_id) " .
                "VALUES ('$goods_id', '$val', '$_SESSION[admin_id]')";
        $db->query($sql);
    }

    $arr = get_goods_member($goods_id);
    $opt = array();
...
//-- 删除指定会员
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_goods_member')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $sql = "DELETE FROM " .$ecs->table('goods_user') . " WHERE " . db_create_in($users, "user_id") . " AND goods_id = '$goods_id'";
    $db->query($sql);
...
/*------------------------------------------------------ */
//卧夕阳工作室 by http://www.51shopla.com   -----结束-----
//QQ：1986219154 
/*------------------------------------------------------ */
